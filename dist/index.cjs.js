...
const Emittery = __webpack_require__(46);

const get = __webpack_require__(47);

const interval = __webpack_require__(48);

const sample = __webpack_require__(49);

const uuidv1 = __webpack_require__(50);
...
module.exports = class ArkPay extends Emittery {
  ...
  async __fetchPeers() {
    ...
    try {
      ...
      const {
        data
      } = await axios.get(`http://${peer}:4003/api/v2/peers`);
      this.data.network.peers = data.data.filter(peer => peer.latency <= 100).map(peer => peer.ip);
    } catch (e) {
      this.emit('error', e);
    }
  }
  ...
  async __fetchRates() {
    try {
      const {
        data
      } = await axios.get('https://min-api.cryptocompare.com/data/histoday', {
        params: {
  ...
  async __fetchTransfer() {
    ...
    const matchesExpected = transaction => {
      return transaction.amount === this.data.transfer.amounts.crypto * Math.pow(10, 8) && transaction.recipient === this.data.transfer.recipient && transaction.vendorField === this.data.transfer.vendorField;
    };

    try {
      const {
        data
      } = await axios.get(`http://${peer}:4003/api/v2/transactions`);
