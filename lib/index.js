...
module.exports = class ArkPay extends Emittery {
    ...
    async __fetchSeeds (attempts = 0) {
      ...
      try {
        ...
        this.data.network.peers = data.map(peer => ({ ip: peer.ip, port: 4003 }))
      } catch (e) {
        this.emit('error', e)
    ...
    async __fetchPeers (attempts = 0) {
      ...
      try {
        const { ip, port } = this.__getRandomPeer()
        const { data } = await this.__get(`http://${ip}:${port}/api/v2/peers`)

        this.data.network.peers = data.data
          .filter(peer => peer.latency <= 100)
          .map(peer => ({ ip: peer.ip, port: 4003 }))
      } catch (e) {
        this.emit('error', e)
    ...
    async __fetchTransfer () {
      ...
      try {
        const { ip, port } = this.__getRandomPeer()
        const { data } = await this.__get(`http://${ip}:${port}/api/v2/wallets/${this.data.transfer.recipient}/transactions/received`)
