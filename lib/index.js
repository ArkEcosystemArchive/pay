...
module.exports = class ArkPay extends Emittery {
    ...
    async __fetchPeers (attempts = 0) {
      ...
      try {
        const { data } = await axios.get(`http://${this.__getRandomPeer()}:4003/api/v2/peers`)
    ...
    async __fetchTransfer () {
      ...
      try {
        const { data } = await axios.get(`http://${this.__getRandomPeer()}:4003/api/v2/wallets/${this.data.transfer.recipient}/transactions/received`)
