...
module.exports = class ArkPay extends Emittery {
    ...
    async __fetchPeers (attempts = 0) {
      ...
      try {
        const { data } = await this.__get(`http://${this.__getRandomPeer()}:4003/api/peers`)
    ...
    async __fetchTransfer () {
      ...
      try {
        const { data } = await this.__get(`http://${this.__getRandomPeer()}:4003/api/wallets/${this.data.transfer.recipient}/transactions/received`)
    ...
    async __get (url, params = {}) {
      return axios.get(url, {
        params,
        headers: {
          'Accept': `application/vnd.arkCoreApi.v2`
        }
      })
    }
}
